...
for idx,line in enumerate(tqdm(sentences,desc='zapping bad strings')):
    for ws in weird_strings:
        if ws[0] in ' '.join(sentences[idx][2]):
            changed_docs.append([sentences[idx][0], sentences[idx][1], ws[0], ws[1]])
...
for idx,line in enumerate(tqdm(sentences,desc='looking for stratigraphic entities')):
    ...
    for idx,strat_phrase,strat_phrase_cut,strat_flag, doc_id, sent_id, max_word_id, min_word_id, strat_name_id,int_name,int_id, sentence in complete_phrase:
        ...
        cursor.execute(""" 
            ...
            (doc_id, sent_id, strat_phrase,strat_phrase_cut, strat_flag, min_word_id, max_word_id, strat_name_id,int_name,int_id, sentence)
        )       
